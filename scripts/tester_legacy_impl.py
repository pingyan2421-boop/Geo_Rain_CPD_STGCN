...
def multi_pred(sess, y_pred, seq, batch_size, n_his, n_pred, step_idx, dynamic_batch=True):
    ...
    pred_array_file = 'output/pred_array.npy'
    np.save(pred_array_file, pred_array)
...
def model_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path='./output/models/'):
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        # Define file paths
        y_test_file = 'output/y_test.npy'
        x_test_file = 'output/x_test.npy'
        evl_file = 'output/evl.npy'

        # Save y_test, x_test, and evl to local files
        np.save(y_test_file, y_test)
        np.save(x_test_file, x_test[0:len_test, step_idx + n_his, :, :])
        np.save(evl_file, evl)
