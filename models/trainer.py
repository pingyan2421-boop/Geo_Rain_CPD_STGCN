...
def model_train(inputs, blocks, args, sum_path='./output/tensorboard'):
    ...
    copy_losses = tf.get_collection('copy_loss')
    if copy_losses:
        copy_loss = tf.add_n(copy_losses)
    else:
        copy_loss = 0.0
    ...
    os.makedirs(pjoin(sum_path, 'train'), exist_ok=True)
    # 确保模型保存目录存在
    os.makedirs(pjoin('./output/models', args.graph), exist_ok=True)
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch):
            ...
            for j, x_batch in enumerate(
                ...
                x_feed = x_batch[:, 0:n_his, :, :]  # 只取历史部分
                summary, _ = sess.run([merged, train_op], feed_dict={x: x_feed, keep_prob: 1.0})

                # 计算 loss
                loss_value = sess.run([train_loss, copy_loss],
                                      feed_dict={x: x_feed, keep_prob: 1.0})
                writer.add_summary(summary, i * epoch_step + j)
                if j % 50 == 0:
                    loss_value = \
                        sess.run([train_loss, copy_loss],
                                 feed_dict={x: x_batch[:, 0:n_his + 1, :, :], keep_prob: 0.5})
